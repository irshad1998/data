...
function encrypt(data: string, encryptionKey: Buffer): { iv: Buffer, encryptedData: string } {
  ...
  function decrypt(encryptedData: string, encryptionKey: Buffer, iv: Buffer): string {
                    ...
                    if (otpVerified === false) {
                        if (expiry > currentDate) {
                            ...
                            if (updateOtpStatusQueryResult?.queryResponse) {
                                ...
                                const og = unmixIV(mixedIv, ivMixKey)
...
function mixIV(ivHex: string, secretKey: string): string {
  ...
  // Function to unmix the mixed string
  function unmixIV(mixedHex: string, secretKey: string): string {
...
export async function verifyUserTokenMw(req: any, res: any){
    try {
         ...
         if(token && apiKey){
            ...
            const mixedIv = apiKey;
            console.log(apiKey);
            
            const unMixedIv = unmixIV(mixedIv, ivMixKey);

            console.log(unMixedIv);
            

            const ivBuffer : Buffer = Buffer.from(unMixedIv, 'hex');

            const decryptedData = decrypt(token, keyBuffer, ivBuffer);

            const userData : UserToken =  JSON.parse(decryptedData);
